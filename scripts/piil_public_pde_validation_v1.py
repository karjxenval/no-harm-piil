#!/usr/bin/env python3
r"""
Paper-grade No-Harm PIIL validation on small public PINN/PDE benchmark files.
...
Example
-------
python scripts/piil_public_pde_validation_v1.py ^
...
def build_candidates(
    ...
    cands: Dict[str, Tuple[str, np.ndarray, str]] = {
        ...
        "underfit_oversmooth": ("underfit", underfit, "oversmoothed candidate; may look stable but lose dynamics"),
    }
...
def run_trial(
    ...
    # Select among all accepted candidates by smallest radius, but baseline remains fallback.
    accepted = [name for name in candidates if metrics[name]["radius"] <= baseline_radius + eps_safe]
    if accepted:
        chosen = min(accepted, key=lambda n: metrics[n]["radius"])
    else:
        chosen = baseline_name

    # Reference data-only and best physics candidate for necessity diagnostics.
    data_only_name = "data_only_lowrank"
    physics_names = [n for n, (fam, _, _) in candidates.items() if fam == "physics_informed"]
    ...
    for name, (family, field, note) in candidates.items():
        m = metrics[name]
        accepted_by_noharm = bool(m["radius"] <= baseline_radius + eps_safe)
...
def write_latex_tables(dataset_summary: pd.DataFrame, necessity_summary: pd.DataFrame, tab_dir: Path) -> None:
    ...
    main = main.rename(columns={
        ...
        "mean_safe_gain_percent": "Safe gain (\\%)",
    ...
    with open(tab_dir / "manuscript_main_table.tex", "w", encoding="utf-8") as f:
        f.write(main.to_latex(index=False, float_format=lambda x: f"{x:.3f}", escape=False))

    nec = necessity_summary.copy()
    # Compact table: average over datasets by observation/noise regime.
    compact = (
    ...
    with open(tab_dir / "manuscript_necessity_table.tex", "w", encoding="utf-8") as f:
        f.write(compact.to_latex(index=False, float_format=lambda x: f"{x:.3f}", escape=False))
