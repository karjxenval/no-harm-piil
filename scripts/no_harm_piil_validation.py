...
def experiment_inverse_heat(fig_dir: str, rng: np.random.Generator) -> Tuple[List[CandidateResult], List[Dict[str, float]]]:
    ...
    # Figure: reconstructions at the hardest final time.
    # The loop variables below are intentionally the last-time reconstructions
    # because `times` is ordered increasingly and Python preserves loop values.
    fig, ax = plt.subplots(figsize=(7.2, 4.2))
    ax.plot(x, q_true, label="true initial condition", linewidth=2.4)
    ax.plot(x, q_base, label="baseline", linestyle="--")
    ax.plot(x, q_good, label="learned good")
    ax.plot(x, q_hallucinated, label="learned high-frequency")
    ax.plot(x, q_bad, label="learned shifted")
    ax.set_xlabel("x")
    ax.set_ylabel("initial condition")
    ax.set_title(f"Inverse heat: reconstructions at hardest time T={times[-1]:.2f}")
    ax.legend(ncol=2)
    savefig(fig, fig_dir, "heat_initial_condition_reconstructions_hardest_time")
    plt.close('all')
...
def aggregate_plots(summary: pd.DataFrame, fig_dir: str) -> None:
    ...
    labels = cov["experiment"].tolist()
    xpos = np.arange(len(labels))
    ax.bar(xpos, cov["covered"])
    ax.set_xticks(xpos)
    ax.set_xticklabels(labels, rotation=25, ha="right")
    ax.set_ylim(0, 1.05)
